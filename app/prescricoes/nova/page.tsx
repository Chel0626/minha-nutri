...
export default function CriarPrescricao() {
  ...
  const atualizarItem = (refeicaoId: string, opcaoId: string, itemId: string, campo: 'quantidade' | 'nome', valor: string) => {
    setRefeicoes(
      refeicoes.map((r) => {
        if (r.id === refeicaoId) {
          return {
  ...
  const handleSalvarPrescricao = async (e: React.FormEvent) => {
    ...
    try {
      ...
      refeicoes.forEach((ref) => {
        ...
        // Loop pelas opções e itens
        ref.opcoes.forEach((opcao, idx) => {
          // Verifica se a opção tem pelo menos um item preenchido
          ...
          if (temItensPreenchidos) {
            ...
            opcao.itens.forEach(item => {
              if (item.nome.trim() || item.quantidade.trim()) {
                // Junta quantidade e nome. Ex: "1 un - Ovo" ou "30g de Queijo"
      ...
      setRefeicoes([{ id: 'ref-1', nome: 'Café da Manhã', metaCarboidratos: '', metaProteinas: '', opcoes: [{ id: 'op-1', itens: [{ id: 'it-1', quantidade: '', nome: '' }] }], observacoes: '', expandido: false }]);
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100">
      <div className="bg-white border-b border-slate-200 shadow-sm">
        <div className="max-w-6xl mx-auto px-6 py-6 flex items-center justify-between">
          ...
          <Link href="/" className="text-emerald-600 hover:text-emerald-700 font-medium text-sm">← Voltar</Link>
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-6 py-8">
        <form onSubmit={handleSalvarPrescricao} className="space-y-6">
          ...
          {success && <div className="bg-green-50 border border-green-200 rounded-lg p-4 flex items-center gap-3 text-green-700"><Check className="w-5 h-5" /> Prescrição salva com sucesso!</div>}
          ...
          <div className="bg-white rounded-lg border border-slate-200 p-8 shadow-sm">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div>
                ...
                <select value={metadados.pacienteId} onChange={handlePacienteChange} className="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-transparent">
              ...
              <div>
                ...
                <input type="text" value={metadados.faseCaloricas} onChange={handleFaseChange} placeholder="Ex: Aprox 1400kcal, 25% prot..." className="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-transparent" />
              </div>
              <div>
                <label className="block text-sm font-semibold text-slate-700 mb-2">Data / Ajuste</label>
                <input type="text" value={metadados.dataPrescricao} onChange={handleDataChange} placeholder="Ex: 06/06 (ajuste calórico...)" className="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-transparent" />
              </div>
            </div>
          </div>
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <button type="button" onClick={adicionarRefeicao} className="flex items-center gap-2 px-4 py-2 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 font-medium">
              ...
              <div key={refeicao.id} className="bg-white rounded-lg border border-slate-200 p-6 shadow-sm">
                ...
                <div className="flex items-center justify-between mb-6 pb-4 border-b border-slate-200">
                  <div className="flex-1">
                    ...
                    <input type="text" value={refeicao.nome} onChange={(e) => atualizarRefeicao(refeicao.id, 'nome', e.target.value)} placeholder="Ex: Café da Manhã e Lanche da Tarde" className="px-4 py-2 border border-slate-300 rounded-lg w-full focus:ring-2 focus:ring-emerald-500" />
                  </div>
                  {refeicoes.length > 1 && (
                    <button type="button" onClick={() => removerRefeicao(refeicao.id)} className="ml-4 text-red-600 hover:text-red-700"><Trash2 className="w-5 h-5" /></button>
                  )}
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
                  <div>
                    ...
                    <input type="number" value={refeicao.metaCarboidratos} onChange={(e) => atualizarRefeicao(refeicao.id, 'metaCarboidratos', e.target.value)} placeholder="Ex: 30" className="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-emerald-500" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-slate-700 mb-1">Meta Proteínas (g)</label>
                    <input type="number" value={refeicao.metaProteinas} onChange={(e) => atualizarRefeicao(refeicao.id, 'metaProteinas', e.target.value)} placeholder="Ex: 18" className="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-emerald-500" />
                  </div>
                </div>
                ...
                <div className="space-y-6 mb-8">
                  {refeicao.opcoes.map((opcao, idxOpcao) => (
                    <div key={opcao.id} className="bg-slate-50 border border-slate-200 rounded-lg p-4">
                      <div className="flex items-center justify-between mb-4">
                        <h4 className="font-bold text-slate-700 uppercase tracking-wider text-sm">Opção {idxOpcao + 1}</h4>
                        {refeicao.opcoes.length > 1 && (
                          <button type="button" onClick={() => removerOpcao(refeicao.id, opcao.id)} className="text-red-400 hover:text-red-600 text-sm flex items-center gap-1">
                            <Trash2 className="w-4 h-4" /> Remover Opção
                          </button>
                        )}
                      </div>

                      {/* Itens (Quantidade + Alimento) */}
                      <div className="space-y-3">
                        <div className="flex gap-2 px-1">
                          <div className="w-24 text-xs font-semibold text-slate-500 uppercase">Qtd / Medida</div>
                          <div className="flex-1 text-xs font-semibold text-slate-500 uppercase">Alimento (Texto Livre por enquanto)</div>
                        </div>
                        ...
                        {opcao.itens.map((item) => (
                          <div key={item.id} className="flex items-center gap-2">
                            ...
                            <input
                              ...
                              placeholder="Ex: 100g, 2 fatias..."
                              className="w-32 px-3 py-2 border border-slate-300 rounded-md focus:ring-2 focus:ring-emerald-500 text-sm"
                            />
                            <input
                              type="text"
                              value={item.nome}
                            ...
                            />
                            <button type="button" onClick={() => removerItem(refeicao.id, opcao.id, item.id)} className="p-2 text-slate-400 hover:text-red-600">
                      ...
                      <div className="mt-4">
                        <button type="button" onClick={() => adicionarItem(refeicao.id, opcao.id)} className="text-emerald-600 font-medium text-sm flex items-center gap-1 hover:text-emerald-700 bg-emerald-100/50 px-3 py-1.5 rounded-md transition">
                  ...
                  <button type="button" onClick={() => adicionarOpcao(refeicao.id)} className="w-full py-3 border-2 border-dashed border-slate-300 text-slate-500 rounded-lg hover:border-emerald-500 hover:text-emerald-600 font-medium transition flex items-center justify-center gap-2">
                ...
                {/* Condutas */}
                <div className="mb-4">
                  <label className="block text-sm font-semibold text-slate-700 mb-2">Observações / Condutas da Refeição</label>
                  <textarea value={refeicao.observacoes} onChange={(e) => atualizarRefeicao(refeicao.id, 'observacoes', e.target.value)} placeholder="Orientações específicas..." rows={2} className="w-full px-4 py-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-emerald-500 resize-none" />
                </div>

                <button type="button" onClick={() => toggleExpandido(refeicao.id)} className="w-full flex items-center justify-between bg-slate-50 border border-slate-200 px-4 py-3 rounded-lg">
                  <span className="font-medium text-slate-900">Condutas Padrão</span>
                  <ChevronDown className={`w-5 h-5 text-slate-600 transition-transform ${refeicao.expandido ? 'rotate-180' : ''}`} />
                </button>
                          ...
                          <div className="space-y-2 pl-2">
                            {configs.map((config) => (
                              <label key={config.id} className="flex items-start gap-3 cursor-pointer hover:bg-white p-2 rounded transition">
                                <input type="checkbox" checked={refeicao.observacoes.includes(config.conteudo)} onChange={(e) => toggleConduta(refeicao.id, config.conteudo, e.target.checked)} className="w-4 h-4 mt-1 text-emerald-600 border-slate-300 rounded" />
                                <div className="flex-1"><p className="text-sm font-medium text-slate-900">{config.titulo}</p></div>
                              </label>
                            ))}
                          </div>
                        </div>
          ...
          <div className="bg-white rounded-lg border border-slate-200 p-8 shadow-sm">
            ...
            <div className="space-y-3">
              <label className="flex items-center gap-3 cursor-pointer p-3 hover:bg-slate-50 rounded-lg"><input type="checkbox" checked={tabelasSelecionadas.proteinas} onChange={() => toggleTabela('proteinas')} className="w-5 h-5 text-emerald-600 rounded" /><span className="font-medium text-slate-900">Tabela 1: Proteínas</span></label>
              <label className="flex items-center gap-3 cursor-pointer p-3 hover:bg-slate-50 rounded-lg"><input type="checkbox" checked={tabelasSelecionadas.substitutosArroz} onChange={() => toggleTabela('substitutosArroz')} className="w-5 h-5 text-emerald-600 rounded" /><span className="font-medium text-slate-900">Tabela 2: Substitutos de Arroz</span></label>
              <label className="flex items-center gap-3 cursor-pointer p-3 hover:bg-slate-50 rounded-lg"><input type="checkbox" checked={tabelasSelecionadas.frutas} onChange={() => toggleTabela('frutas')} className="w-5 h-5 text-emerald-600 rounded" /><span className="font-medium text-slate-900">Tabela 3: Frutas</span></label>
            </div>
          </div>

          <div className="flex gap-4 justify-end">
            <Link href="/" className="px-6 py-3 border border-slate-300 rounded-lg text-slate-900 font-medium">Cancelar</Link>
            <button type="submit" disabled={loading} className="px-6 py-3 bg-emerald-600 text-white rounded-lg font-medium hover:bg-emerald-700 disabled:bg-slate-400">
